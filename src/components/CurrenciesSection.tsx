    ...
    <section className="py-12 sm:py-16 lg:py-20" style={{ backgroundColor: colors.background }}>
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
        <div className="text-center mb-8 sm:mb-12 animate-fadeIn px-2">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-2 sm:mb-4" style={{ color: colors.text }}>35+ Supported Currencies</h2>
          <p className="text-base sm:text-lg lg:text-xl font-medium max-w-2xl mx-auto" style={{ color: colors.text, opacity: 0.8 }}>Deposit and withdraw in your local currency</p>
        </div>
        
        <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 sm:gap-4 px-2 sm:px-0">
          {Object.entries(currencies).slice(0, 11).map(([code, currency], index) => (
            <div 
              key={code} 
            ...
            >
              <div className="text-3xl sm:text-4xl mb-1 sm:mb-2">{currency.flag}</div>
              <div className="font-bold text-xs sm:text-sm" style={{ color: colors.text }}>{code}</div>
              <div className="text-[10px] sm:text-xs mt-0.5 font-medium" style={{ color: colors.text, opacity: 0.7 }}>{currency.name}</div>
            </div>
          ))}
          <div 
            className="rounded-lg sm:rounded-xl p-2 sm:p-3 text-center border hover:scale-105 sm:hover:scale-110 hover:shadow-md sm:hover:shadow-lg transition-all duration-300 animate-scaleIn" 
          ...
          >
            ...
            <div className="font-bold text-xs sm:text-sm" style={{ color: colors.text }}>+25 More</div>
            <div className="text-[10px] sm:text-xs mt-0.5 font-medium" style={{ color: colors.text, opacity: 0.7 }}>Global Coverage</div>
          </div>
        </div>
        
        {/* Digital Currency Status */}
        <div className="mt-12 sm:mt-16 rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 shadow-lg sm:shadow-xl border animate-scaleIn" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
          <div className="text-center mb-8 sm:mb-12">
            <h3 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4" style={{ color: colors.text }}>Digital Currency Support</h3>
            <p className="text-base sm:text-lg font-medium max-w-2xl mx-auto" style={{ color: colors.text, opacity: 0.8 }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="text-center mb-4 sm:mb-6">
                <h4 className="text-xl sm:text-2xl font-bold flex items-center justify-center gap-2 sm:gap-3 mb-1 sm:mb-2" style={{ color: colors.text }}>
                  <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-green-500 rounded-full animate-pulse shrink-0"></span>
                  Currently Active
                </h4>
                <p className="text-xs sm:text-sm font-medium" style={{ color: colors.text, opacity: 0.7 }}>Live and available for use</p>
              </div>
              
              <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 hover:shadow-md sm:hover:shadow-lg transition-all duration-300" style={{ backgroundColor: colors.successBg, borderColor: colors.successBorder }}>
                <div className="flex flex-col items-center text-center">
                  <img src={icons.cUSD} alt="cUSD" className="h-14 w-14 sm:h-16 sm:w-16 mb-3 sm:mb-4"/>
                  <div className="mb-2 sm:mb-3">
                    <div className="font-bold text-base sm:text-lg" style={{ color: colors.text }}>Celo Dollar (cUSD)</div>
                    <div className="text-xs sm:text-sm font-medium" style={{ color: colors.text, opacity: 0.75 }}>Pegged to US Dollar</div>
                  </div>
                  <span className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold whitespace-nowrap" style={{ backgroundColor: colors.primary, color: colors.surface }}>Active & Live</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="text-center mb-4 sm:mb-6">
                ...
                <p className="text-xs sm:text-sm font-medium" style={{ color: colors.text, opacity: 0.7 }}>Expanding global coverage</p>
              </div>
